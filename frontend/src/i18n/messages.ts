...
export const messages: Record<Lang, Dict> = {
  en: {
    ...
    systemSettingsLead: 'Manage cross-provider routing, check-in defaults, updates, SQLite protection, and the console API key.',
    ...
    workbuddyCheckinSettingsTitle: 'Daily check-in',
    workbuddyCheckinSettingsHint: 'Default first daily check-in time for new WorkBuddy and Trae accounts. Existing accounts keep their own time until you edit them.',
    workbuddyCheckinSettingsFieldHint: 'Uses server local time. New WorkBuddy and Trae accounts inherit this unless you set a different time on the account.',
    ...
    autoCheckinHint: 'Run the daily credit check-in at the configured local time. Failed attempts retry around 21:00. Off by default.',
    autoCheckinCreateHint: 'WorkBuddy and Trae. Enable automatic daily credit check-ins. The first attempt uses this account time, or the system default if left unchanged.',
    ...
    editAccountHint: 'Change the display name, concurrency, routing priority, and check-in time.',
  ...
  zh: {
    ...
    systemSettingsLead: '管理跨 Provider 路由、签到默认时间、系统更新、SQLite 保护和控制台管理员密钥。',
    ...
    workbuddyCheckinSettingsTitle: '每日签到',
    workbuddyCheckinSettingsHint: '新建 WorkBuddy 与 Trae 账号时使用的默认首次签到时间。已有账号会保留各自的时间，直到你单独修改。',
    workbuddyCheckinSettingsFieldHint: '使用服务器本地时间。新建 WorkBuddy 与 Trae 账号会继承这里的时间，账号编辑页仍可单独覆盖。',
    ...
    autoCheckin: '每日签到',
    autoCheckinHint: '按账号配置的本地时间自动领取积分；失败会在约 21:00 重试。默认关闭。',
    autoCheckinCreateHint: '支持 WorkBuddy 与 Trae。开启后按该账号的签到时间自动领取积分；未改时间时使用系统默认。',
    ...
    editAccountHint: '修改显示名称、并发、调度优先级，以及签到时间。',
