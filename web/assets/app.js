const state = {
  overview: null,
  page: "home",
  lang: localStorage.getItem("cli2api_lang") || "zh",
};

function t(key, vars = {}) {
  const pack = (window.CLI2API_I18N && (window.CLI2API_I18N[state.lang] || window.CLI2API_I18N.en)) || {};
  let s = pack[key];
  if (s == null && window.CLI2API_I18N?.en) s = window.CLI2API_I18N.en[key];
  if (s == null) s = key;
  return String(s).replace(/\{(\w+)\}/g, (_, k) => (vars[k] != null ? vars[k] : `{${k}}`));
}

function pageCopy(name) {
  const pack = (window.CLI2API_I18N && (window.CLI2API_I18N[state.lang] || window.CLI2API_I18N.en)) || {};
  return (pack.pages && pack.pages[name]) || { kicker: name, title: name, desc: "" };
}
...
function setLang(lang) {
  ...
  localStorage.setItem("cli2api_lang", state.lang);
  applyStaticI18n();
  switchPage(state.page);
  if (state.overview) {
    renderHome(state.overview);
    renderEndpoints(state.overview);
    renderAuth(state.overview);
    renderModels(state.overview);
  }
}
...
function renderAuth(overview) {
  ...
  $("auth-kv").innerHTML = `
    <div><span>${t("hasUserBlob")}</span><b>${auth.has_user_blob ? t("yes") : t("no")}</b></div>
    <div><span>${t("userBlobBytes")}</span><b>${auth.user_blob_bytes ?? 0}</b></div>
    <div><span>${t("machineId")}</span><b>${auth.machine_id || "—"}</b></div>
    <div><span>${t("workerHot")}</span><b>${worker.hot ? t("yes") : t("no")}</b></div>
    <div><span>${t("workerEndpoint")}</span><b>${worker.endpoint || "—"}</b></div>
    <div><span>${t("rewarmCount")}</span><b>${worker.rewarmCount ?? worker.rewarm_count ?? 0}</b></div>
  `;
  const status = login.status || "idle";
  $("login-status").textContent = status === "idle" ? t("idle") : status;
  $("login-message").textContent = login.message || t("readyWhen");
  if (login.authUrl) {
...
function renderModels(overview) {
  ...
  if (!filtered.length) {
    $("model-table").innerHTML = `<div class="empty">${models.length ? t("noModelsMatch") : t("noProviders")}</div>`;
  } else {
    $("model-table").innerHTML = `
    <div class="tr head"><div>${t("modelCol")}</div><div>${t("mappedKeyCol")}</div><div>${t("stateCol")}</div></div>
    ${filtered
      .map(
        (m, idx) => `
      <div class="tr" style="animation-delay:${idx * 20}ms">
        ...
        <div><span class="chip">${m.stale ? t("fallback") : t("ready")}</span></div>
      </div>`,
      )
      .join("")}
  `;
...
function renderHome(overview) {
  ...
  $("m-runtime").textContent = proxyOk ? t("running") : t("down");
  $("m-worker").textContent = workerOk ? (hot ? t("hot") : t("up")) : t("down");
  $("m-auth").textContent = authOk ? t("ready") : t("missing");
  $("m-port").textContent = overview?.proxy?.port || location.port || "3010";
  $("home-updated").textContent = overview?.time || new Date().toLocaleString();
  $("home-kv").innerHTML = `
    <div><span>${t("proxy")}</span><b>${proxyOk ? "ok" : t("down")}</b></div>
    <div><span>${t("workerHot")}</span><b>${hot ? "true" : "false"}</b></div>
    <div><span>${t("endpoint")}</span><b>${overview?.worker?.endpoint || overview?.proxy?.chat_url || "—"}</b></div>
    <div><span>${t("rewarm")}</span><b>${String(overview?.worker?.rewarmCount ?? overview?.worker?.rewarm_count ?? 0)}</b></div>
    <div><span>${t("lastError")}</span><b id="kv-error">${overview?.worker?.lastError || overview?.worker?.last_error || "—"}</b></div>
  `;
  setStatus($("sidebar-runtime"), proxyOk && workerOk ? t("running") : t("degraded"), proxyOk && workerOk ? "ok" : "warn");
}
...
async function refresh() {
  ...
  try {
    ...
    setStatus($("sidebar-runtime"), t("error"), "bad");
    $("m-runtime").textContent = t("error");
    $("endpoint-list").innerHTML = `<div class="empty">${t("failedOverview", { msg: err.message })}</div>`;
    $("home-kv").innerHTML = `<div><span>${t("lastError")}</span><b>${err.message}</b></div>`;
    console.error(err);
  } finally {
...
function switchPage(name) {
  ...
  $("page-kicker").textContent = copy.kicker;
  $("page-title").textContent = copy.title;
  $("page-desc").textContent = copy.desc;
}

async function rewarm() {
  $("btn-rewarm").textContent = t("rewarming");
  try {
    ...
    $("btn-rewarm").textContent = t("rewarm");
  }
}
...
async function testChat() {
  ...
  $("test-out").textContent = t("requesting");
  try {
...
async function startDeviceLogin() {
  $("btn-login-device").textContent = t("starting");
  try {
    ...
    if (out.authUrl) {
      ...
      $("login-message").textContent = out.message || t("loginOpenMsg");
    ...
    $("btn-login-device").textContent = t("startBrowserLogin");
  }
}
...
async function loginWithPat() {
  ...
  if (!pat) {
    alert(t("pastePatFirst"));
    return;
  }
  $("btn-login-pat").textContent = t("loggingIn");
  try {
    ...
    $("login-message").textContent = t("patDone");
    $("pat-input").value = "";
    await refresh();
  } catch (err) {
    alert(err.message);
  } finally {
    $("btn-login-pat").textContent = t("loginWithPat");
  }
}

async function refreshModels() {
  $("btn-refresh-models").textContent = t("refreshing");
  try {
    ...
    $("btn-refresh-models").textContent = t("refresh");
  }
}
...
function boot() {
  ...
  applyStaticI18n();
  if ($("lang-zh")) $("lang-zh").onclick = () => setLang("zh");
  if ($("lang-en")) $("lang-en").onclick = () => setLang("en");
  document.querySelectorAll(".nav-item").forEach((btn) => {
  ...
  $("btn-copy-login").onclick = async () => {
    ...
    $("btn-copy-login").textContent = t("copied");
    setTimeout(() => ($("btn-copy-login").textContent = t("copy")), 900);
  };
  ...
  $("btn-copy-openai").onclick = async () => {
    const url = state.overview?.access?.openai_base_url
      ...
      : `${originBase()}/v1`;
    await navigator.clipboard.writeText(url);
    $("btn-copy-openai").textContent = t("copied");
    setTimeout(() => ($("btn-copy-openai").textContent = t("copyBaseUrl")), 900);
  };
